...
# --------------------------------------------------------------------------
# [0] Flow API 설정 (★여기에 확인하신 ID 숫자를 입력하세요★)
# --------------------------------------------------------------------------
# 예: NOTICE_PROJECT_ID = 123456
NOTICE_PROJECT_ID = 0  # [KCIM] 전체 공지사항 ID 입력
HR_PROJECT_ID = 0      # [경영본부] HR팀 ID 입력

# API 키는 secrets.toml 또는 입력란에서 관리 (여기서는 secrets 사용 권장)
# .streamlit/secrets.toml 파일에 [flow_api] api_key = "..." 저장 필요
try:
    FLOW_API_KEY = st.secrets["flow_api"]["api_key"]
except:
    FLOW_API_KEY = "" # 없으면 작동 안 함
...
# --------------------------------------------------------------------------
# [2] 지식 베이스 (Flow 연동 + 정적 데이터)
# --------------------------------------------------------------------------
@st.cache_data(ttl=1800) # 30분마다 갱신
def fetch_flow_data():
    if not FLOW_API_KEY or NOTICE_PROJECT_ID == 0:
        return ""
    
    headers = {"Authorization": f"Bearer {FLOW_API_KEY}", "Content-Type": "application/json", "Accept": "application/json"}
    collected_text = "\n[실시간 Flow 공지사항]\n"
    
    targets = [("[KCIM] 전체 공지사항", NOTICE_PROJECT_ID), ("[경영본부] HR팀", HR_PROJECT_ID)]
    
    for title, pid in targets:
        if pid == 0: continue
        try:
            url = f"https://kcim.flow.team/api/v1/projects/{pid}/posts"
            res = requests.get(url, headers=headers, timeout=5)
            if res.status_code == 200:
                posts = res.json().get('result', [])
                collected_text += f"\n<{title} 최신글>\n"
                for p in posts[:3]: # 최신글 3개만
                    collected_text += f"- {p.get('title', '제목없음')}: {p.get('content', '')[:100]}...\n"
        except: pass
        
    return collected_text

# Flow 데이터 가져오기
flow_updates = fetch_flow_data()

COMPANY_DOCUMENTS_INFO = flow_updates + """
...
def summarize_text(text):
    ...
    try:
        ...
        res = client.chat.completions.create(model="gpt-4o-mini", messages=[{"role": "system", "content": "핵심 요약 전문가. 명사형 종결."},{"role": "user", "content": text}], temperature=0)
...
if not st.session_state["logged_in"]:
    ...
    with st.sidebar:
        st.markdown(f"""<div style="background-color: white; padding: 20px; border-radius: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); text-align: center; margin-bottom: 25px;"><div style="color: #868e96; font-size: 13px; margin-bottom: 5px;">인증된 임직원</div><div style="color: #212529; font-size: 20px; font-weight: 800;">{user['name']} {user['rank']}</div><div style="background-color: #e7f5ff; color: #1c7ed6; font-size: 13px; font-weight: 700; display: inline-block; padding: 4px 12px; border-radius: 15px; margin-top: 8px;">{user['dept']}</div></div>""", unsafe_allow_html=True)
        ...
        if user['name'] in ["관리자", "이경한"]:
            ...
            st.markdown(f"""<a href="{sheet_url}" target="_blank" style="text-decoration: none;"><div style="background-color: #f8f9fa; padding: 15px; border-radius: 12px; border: 1px solid #dee2e6; text-align: center; margin-bottom: 25px; transition: 0.3s; cursor: pointer;"><span style="font-size: 22px;">📊</span><br><span style="font-weight: bold; color: #495057; font-size: 15px;">민원 현황 시트</span><br><span style="font-size: 11px; color: #adb5bd;">Google Sheets 이동</span></div></a>""", unsafe_allow_html=True)

        st.caption("문의하실 주제를 선택하세요")
        cats = [("🛠️ 시설/수리", "유지보수"), ("👤 인사/채용", "제증명/발령"), ("📋 규정/보안", "사내규정"), ("🎁 복지/휴가", "지원금/휴가"), ("📢 불편사항", "고충 접수"), ("💬 일반/기타", "단순 문의")]
        
        for title, desc in cats:
            if st.button(f"{title} | {desc}", key=title, disabled=st.session_state["inquiry_active"], use_container_width=True):
                ...
                for f_name in RULES_LIST:
                    if f_name in msg["content"]:
                        ...
                        if os.path.exists(path):
                            ...
                            st.download_button(label=f"📂 {f_name} 다운로드", data=file_data, file_name=f_name, mime="application/octet-stream", key=f"dl_{f_name}_{msg['content'][:5]}")
    ...
    if prompt := st.chat_input("문의 내용을 입력하세요"):
        ...
        2. "파일을 보라"는 말 금지. 지식 베이스 내용을 바탕으로 직접 설명해.
                ...
                with st.chat_message("assistant"):
                    ...
                    for f_name in RULES_LIST:
                        if f_name in clean_ans:
                            ...
                            if os.path.exists(path):
                                with open(path, "rb") as f: file_data = f.read()
                                st.download_button(label=f"📂 {f_name} 다운로드", data=file_data, file_name=f_name, mime="application/octet-stream", key=f"new_{f_name}")
