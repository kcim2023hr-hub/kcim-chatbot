...
st.markdown("""
    ...
    div[data-testid="stNotification"] { font-size: 17px; line-height: 1.6; background-color: #f0f7ff; border-radius: 12px; color: #0056b3; padding: 20px; }
    ...
    /* 상담 중 버튼 잠금 스타일 */
    div[data-testid="stSidebar"] .stButton > button:disabled { background-color: #f0f0f0 !important; color: #aaa !important; cursor: not-allowed !important; }
...
# 이경한 매니저는 KICM(KCIM) HR팀에서 시설, 차량, 근태를 관리합니다.
CATEGORY_GREETINGS = {
...
if not st.session_state["logged_in"]:
    ...
    with st.sidebar:
        ...
        if st.session_state["inquiry_active"]:
            if st.button("✅ 상담 종료 및 대화 초기화", use_container_width=True): reset_chat()
    ...
    # [수정] 입력 지연 해결을 위한 답변 생성 로직 통합
    if prompt := st.chat_input("문의 내용을 입력하세요"):
        st.session_state["inquiry_active"] = True
        st.session_state.messages.append({"role": "user", "content": prompt})
        with st.chat_message("user"): st.write(prompt)
        
        # 1990년 창립된 KCIM의 HR 매니저 페르소나 적용
        system_msg = f"너는 1990년 창립된 KCIM의 HR 매니저야. {user['name']}님께 정중히 답변해줘."
        
        # 로딩 스피너 추가하여 입력창 먹통 방지 및 시각적 피드백 제공
        with st.spinner("KCIM 매니저가 답변을 작성 중입니다..."):
            try:
                client = OpenAI(api_key=st.secrets["OPENAI_API_KEY"])
                response = client.chat.completions.create(model="gpt-4o-mini", messages=[{"role": "system", "content": system_msg}] + st.session_state.messages)
