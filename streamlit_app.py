...
def get_dynamic_greeting():
    """현재 접속 시간대별 맞춤형 인사말 생성"""
    now_hour = get_kst_time().hour
    
    if 5 <= now_hour < 11:
        return "좋은 아침입니다! 오늘도 활기차게 시작해볼까요? ☀️"
    elif 11 <= now_hour < 14:
        return "즐거운 점심시간입니다. 맛있는 식사 하셨나요? 🍱"
    elif 14 <= now_hour < 18:
        return "즐거운 오후입니다. 업무 중에 궁금한 점이 있으신가요? ☕"
    elif 18 <= now_hour < 22:
        return "오늘 하루도 고생 많으셨습니다! 마무리하며 도와드릴 일이 있을까요? ✨"
    else:
        return "고요한 밤입니다. 늦은 시간까지 수고가 많으시네요. 무엇을 도와드릴까요? 🌙"

def save_to_sheet(dept, name, rank, category, question, answer, status):
    """구글 시트 기록 로직 (KST 시간 반영)"""
...
def summarize_text(text):
    ...
    try:
        ...
        res = client.chat.completions.create(model="gpt-4o-mini", messages=[{"role": "system", "content": "1문장 핵심 요약."}, {"role": "user", "content": text}], temperature=0)
...
# [챗봇 메인 화면]
    ...
    if prompt := st.chat_input("문의 내용을 입력하세요"):
        ...
        sys_msg = f"너는 1990년 창립된 KCIM의 전문 HR 매니저야. {user['name']}님께 정중하게 답변해줘. 마지막에 반드시 [CATEGORY:분류명]을 포함해줘."
