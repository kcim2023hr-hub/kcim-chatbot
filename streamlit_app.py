...
# --------------------------------------------------------------------------
# [1] 데이터 로드 및 유틸리티 (Saved Info, KST 보정)
# --------------------------------------------------------------------------
...
def summarize_text(text):
    ...
    try:
        ...
        res = client.chat.completions.create(model="gpt-4o-mini", messages=[{"role": "system", "content": "15자 이내 한 줄 요약."}, {"role": "user", "content": text}], temperature=0)
...
if not st.session_state["logged_in"]:
    ...
    if prompt := st.chat_input("문의 내용을 입력하세요"):
        ...
        [답변 원칙]
        1. 사내 규정은 아래 목록을 우선 참고해: {COMPANY_DOCUMENTS_INFO}
        2. 최신 법령(예: 최저임금, 출산지원) 등 외부 정보가 필요하면 'search_web' 도구를 사용해.
        3. 실무 확인 건은 끝에 [ACTION]을 붙이고, 마지막엔 [CATEGORY:분류명]을 포함해줘.
        """
