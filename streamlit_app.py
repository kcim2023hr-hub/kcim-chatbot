...
if not st.session_state["logged_in"]:
    ...
    if prompt := st.chat_input("문의 내용을 입력하세요"):
        ...
        2. 만약 사내 규정에 없는 '법적 기준'이나 '공식 정보'를 물어본다면, 네가 알고 있는 최신 법령(근로기준법 등)과 공신력 있는 자료를 바탕으로 답변해줘.
        3. 외부 정보를 알려줄 때는 출처를 분명하게 표시해줘.
        3. "파일을 확인해보세요", "파일을 참고하세요"라는 말보다, 파일에 담긴 "내용을 설명해주는 것"이 최우선이야.
        4. 사용자가 파일을 열어보지 않아도 답변만으로 충분히 정보를 얻을 수 있어야 해.
        4. 자료가 없거나, 답변이 어려운건 솔직하게 "이 부분은 잘 모르겠습니다. 담당자가 확인할 수 있도록 기록하겠습니다."로 답변
        5. 답변 마지막에 관련 자료가 필요한지 물어보고 요청을 한다면 해당 파일 다운로드 버튼을 제시해줘.
        6. 마지막에 [CATEGORY:분류] 필수.
        {COMPANY_DOCUMENTS_INFO}
